...
def run(user_message: str, conv_history: list[dict]) -> tuple[str, list[dict]]:
    ...
    messages = [{"role": "system", "content": build_system_prompt()}] + conv_history
    ...
    for turn in range(MAX_TURNS):
        ...
        if not msg.tool_calls:
            print(f"  [agent] Answering from knowledge (no tool calls)")
        ...
        if clean_content and msg.tool_calls:
            tool_names = [tc.function.name for tc in msg.tool_calls]
            print(f"  [agent] Draft answer + calling {', '.join(tool_names)} for gaps")
        elif msg.tool_calls:
            tool_names = [tc.function.name for tc in msg.tool_calls]
            print(f"  [agent] Calling {', '.join(tool_names)}")
